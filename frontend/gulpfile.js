...
function browsersync() {
  browserSync.init({
    proxy: {
      target: "http://javascript", // заворачивает локальный адрес в localhost
    },
 ...
 *
 * Чтобы использовать другой препроцессор,
 * нужно прописать внутри .pipe() это - eval(preprocessor)()
 * @returns {*}
 */
function css() {
...
exports.default = parallel(cleanWeb, css, js, img, fonts, faviconGenerate, sprite, browsersync, startWatch)